...
//$type=1;
   ...
   $_SESSION['LOGIN_FAILED'] = "Unknown student! verify if your Reg. No.: $username is correctly typed ";
    ...
    exit();
     ...
     //echo $type; exit;
if($type==3){
    //Ordinaryregular student
    $qrey = "SELECT loginpassword FROM tblstudents WHERE matricnumber='$username' AND loginpassword='$password' limit 1";

}elseif($type==2){
    //sample regular students
    $qrey = "SELECT loginpassword FROM tblsbrsstudents WHERE sbrsno='$username' AND loginpassword='$password' limit 1";
    
}elseif($type==1){
 //utsample student
    $qrey = "SELECT StateOfOrigin FROM tbljamb WHERE RegNo='$username' AND StateOfOrigin='$password' limit 1";

}else{
    // sbrs  entrance candidate. modify query when table available
    $qrey = "SELECT stateOforigin from tblsbrsshortlist where ApplicationID='$username' AND stateOforigin='$password' limit 1";
}
$stmt1=$dbh->prepare($qrey);
$stmt1->execute();
if ($stmt1->rowCount() == 1) {
   ...
   //check if the student registered for the exams he is trying to write
		...
		$qry = "SELECT distinct tblscheduledcandidate.candidateid, venueid FROM  tblscheduledcandidate INNER JOIN tblcandidatestudent ON 
		tblscheduledcandidate.candidateid= tblcandidatestudent.candidateid
		 ...
		 WHERE RegNo='$username' AND tblscheduling.testid='$testid'";
