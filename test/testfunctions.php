<?php
// Include HTML sanitizer for XSS protection
if (file_exists(__DIR__ . '/../lib/html_sanitizer.php')) {
...
$sanitized = function_exists('sanitizeHtmlForRender') ? sanitizeHtmlForRender($decoded) : $decoded;
?>
...
?>">
    <div class="qadiv" style="background-color:#ffffff">
        <div class="questiondiv">
            ...
            <b/><?php echo str_replace("&#160;", "", $sanitized); ?>
        </div>
        <div class="answerdiv">
            <?php
		...
		tblpresentation.testid='$testid' and tblpresentation.candidateid='$candidateid'
			...
			testid='$testid' and candidateid='$candidateid' )";
                                    ...
                                    while ($resultquest = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                        ...
                                        $sanitized_answer = function_exists('sanitizeHtmlForRender') ? sanitizeHtmlForRender($decoded_answer) : $decoded_answer;
                                        $marker = chr(65 + $optioncounter); // A, B, C, D ...
                                        if ($answerid == $tblscoreanswerid) {
                                            echo "<li class='ao selected-opt'>
					<label class='optionlabel'><input type='radio' class=\"answeroption\" name='$questionid' id='$answerid' value='$answerid' checked> <b>$marker.</b> <div style='display:inline-block; cursor:pointer;'> " . str_replace("&#160;", "", $sanitized_answer) . "</div></label> </li>";
                                        } else {
                                            echo "<li class='ao'>  <label class='optionlabel'><input type='radio' class=\"answeroption\"  name='$questionid' id='$answerid' value='$answerid'><b>$marker:</b> <span style='display:inline-block; width:8px;'></span><div style='display:inline-block; cursor:pointer;'>" . str_replace("&#160;", "", $sanitized_answer) . "</div></label> </li>";
                                        }
                                        $optioncounter++;
                                    }
                                    echo "</ol>";
