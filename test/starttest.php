...
// Regenerate session ID for exam security
session_regenerate_id(true);

// Generate CSRF token for exam session if not already set
if (!isset($_SESSION['exam_csrf_token'])) {
    $_SESSION['exam_csrf_token'] = bin2hex(random_bytes(32));
}

// Initialize exam activity tracking
...
$query1 = "SELECT count(distinct(questionid)) as totalquestions FROM tblpresentation where(candidateid=? and testid=?)";
$stmt = $dbh->prepare($query1);
$stmt->execute([$candidateid, $testid]);
...
//create the questions. the question for the student is not yet created
	...
	where(testid=? and tbltestsubject.subjectid in
	(select subjectid from tblcandidatestudent inner join tblscheduling on 
	tblcandidatestudent.scheduleid=tblscheduling.schedulingid where(testid=? and candidateid=?)) )";
    // echo $query2;
    $stmt = $dbh->prepare($query2);
    $stmt->execute([$testid, $testid, $candidateid]);
    $numrows = $stmt->rowCount();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($numrows >= 1) {
...
//	 echo"stud=$totalquestions test=$totaltoanswer";
    if ($totalquestions != $totaltoanswer) {
        ...
        $query3 = "SELECT * FROM tblscore where (candidateid=? and testid=?)";
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Left Navigation Panel - Overridden by navigation-buttons.css */
        #side-widget {
            /* Styles moved to navigation-buttons.css */
        }

        /* Question Content Area - Overridden by navigation-buttons.css */
        .test-questions {
            /* Styles moved to navigation-buttons.css */
        }
...
<body>
    <div id="bannerparent">
        <!-- Banner content will be placed here -->
    </div>
    ...
    <div id="container">
        ...
        <div id="side-widget">
            ...
            <div id="calcbtn">
                <img src="<?php echo siteUrl("assets/img/cbtcalc1.png"); ?>" title='Calculator'>
            </div>
            <?php } ?>
...
?>
<script src="js/modern-dialog.js"></script>
<script type="text/javascript">
    // CSRF token for exam actions
    var examCsrfToken = '<?php echo isset($_SESSION['exam_csrf_token']) ? $_SESSION['exam_csrf_token'] : ''; ?>';
...
//calculator
    $(document).on('click', '#calcbtn', function (event) {
        if(calcInstance){
            $("#calcdiv").dialog('open');
            // Ensure z-index is applied when reopening
            $("#calcdiv").parent().css('z-index', 99999);
        ...
        $("#calcdiv").dialog({
            modal: false,
            title: 'Calculator',
            zIndex: 99999, // Very high z-index to appear on top of all elements
            autoOpen: true,
            width: '250px',
            resizable: false,
            closeOnEscape: true,
            ...
            dialogClass: 'modern-calculator-dialog',
            open: function(event, ui) {
                // Ensure z-index is applied after dialog opens
                var $dialog = $(this).parent();
                $dialog.css('z-index', 99999);
                
                // Keep z-index high on focus/interaction
                $dialog.on('mousedown focus', function() {
    ...
    function cd() {
        ...
        if (remainingsecond <= 0) {
            ...
            $.ajax({
                type: 'POST',
                url: 'updatetime.php',
                data: {endpoint: '1', csrf_token: examCsrfToken}
            }).done(function (msg) {
                //alert(msg);
                //////////////////////////////////////
                const timeExpiredDialog = new ModernDialog({
                    title: 'Computer Based Exams',
                    message: 'Your time is over! Please logout.',
                    type: 'warning',
                    buttons: [
                        {
                            text: 'Logout',
                            ...
                            onClick: function() {
<?php
if (strtoupper(substr($matric, 0, 4)) == "U11A")
...
//update server time
    setInterval(function () {
        ...
        $.ajax({
            type: 'POST',
            url: 'updatetime.php',
            data: {endtime: 2, csrf_token: examCsrfToken}
        }).done(function (msg) {
            //alert(msg);
            if (msg == "end") {
<?php
...
//////
    ...
    $(document).on('click', ".ao", function (evt) {
        ...
        $.ajax({
            ...
            url: 'saveanswer.php',
            timeout: 40000,
            data: {question: dis.attr('name'), ans: dis.val(), csrf_token: examCsrfToken}
        }).done(function (msg) {
            // alert("answer saved");
            ...
            if (parseInt($.trim(msg)) == 0) {
                //////////////////////////////
                const completionDialog = new ModernDialog({
                    title: 'Computer Based Practice',
                    message: 'You have successfully completed the Practice! Would you like to review your Answers or submit?',
                    type: 'success',
                    buttons: [
                        {
                            text: 'Submit',
                            ...
                            onClick: function() {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: 'updatetime.php',
                                    data: {completion: comp, csrf_token: examCsrfToken}
                                }).done(function (msg) {
<?php
if (strtoupper(substr($matric, 0, 4)) == "PUTM")
    echo "window.location.href ='showscore.php';";
else
    echo "window.location.href = 'logout.php';";
?>
                        ...
                        },
                        {
                            ...
                            type: 'secondary',
                            onClick: function() {
                                // Do nothing, just close dialog and continue reviewing
                            }
                        }
                    ]
                });
