...
function is_test_compositor_of($testid = 0, $uid = null, $sbjid = null)
{
    ...
    if (isset($sbjid)) {
        $sbjid = clean($sbjid);
        $query = "select userid from tbltestcompositor where (testid=? && userid=? && subjectid=?)";
        $result = $dbh->prepare($query);
        $result->execute([$testid, $uid, $sbjid]);
    } else {
        $query = "select userid from tbltestcompositor where (testid=? && userid=?)";
        $result = $dbh->prepare($query);
        $result->execute([$testid, $uid]);
    }

    $row = $result->fetch(PDO::FETCH_ASSOC);

    // Check if row exists and matches before accessing array
    if ($row && isset($row['userid']) && $uid == $row['userid']) {
        return true;
    } else {
        return false;
    }
...
//    var_dump($candtype);exit;
    ...
    if ($count) {
        ...
        //echo $candtype;exit;
        // Insert placeholder record with required fields set to 0 (will be updated later during scheduling)
        // This works with MySQL strict mode (PHP 7.4+) which requires all NOT NULL fields to have values
        if ($candtype == 1)
            $sql1 = "insert into tblscheduledcandidate (candidatetype, RegNo, candidateid, scheduleid, subjectid) values (1, UPPER(?), 0, 0, 0)";
        elseif ($candtype == 2)
            $sql1 = "insert into tblscheduledcandidate (candidatetype, RegNo, candidateid, scheduleid, subjectid) values (2, UPPER(?), 0, 0, 0)";
        elseif ($candtype == 3)
            $sql1 = "insert into tblscheduledcandidate (candidatetype, RegNo, candidateid, scheduleid, subjectid) values (3, UPPER(?), 0, 0, 0)";
        elseif ($candtype == 4)
            $sql1 = "insert into tblscheduledcandidate (candidatetype, RegNo, candidateid, scheduleid, subjectid) values (4, UPPER(?), 0, 0, 0)";
        /* else {

        }*/
        $stmt1 = $dbh->prepare($sql1);
        $exec = $stmt1->execute(array($matric));
...
function get_centre_name($cid = 0)
{
    ...
    $cid = clean($cid);
    $cid = intval($cid);
    $query = "select centrename from tblcentres where (centreid=?)";
    $result = $dbh->prepare($query);
...
function get_venue_name($venueid)
{
    ...
    $result->execute(array($venueid));
    $row = $result->fetch(PDO::FETCH_ASSOC);
    return ($row['venuename'] . " (" . $row['location'] . ")");
}
