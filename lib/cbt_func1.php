...
function get_schedule_id($tid, $venueid) {
    global $dbh;
    $s = array(); $query = "select schedulingid from tblscheduling where (testid = ? && venueid=?)"; // echo $sql;
	$result = $dbh->prepare($query);
...
function get_test_schedule_as_array($tid) {
    ...
    $schedules = array();
    $tid = clean($tid);
    $query = "select schedulingid from tblscheduling where (testid = ?)";
	$result = $dbh->prepare($query);
	$result->execute(array($tid));
    $i = 0;
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
...
function get_test_versions_as_array($testid) {global $dbh;
...
function get_previewer_test_as_array($limit = -1) {global $dbh;
    $tests = array();
    $limit = intval(clean($limit));
    $uid = $_SESSION['MEMBER_USERID'];
    if ($limit != -1 && $limit)
...
function get_test_question_count($testid, $test_subject, $version) {global $dbh;
...
function get_mapped_faculty_as_array($schd) {global $dbh;
    $query = "select facultyid from tblfaculty_schedule_mapping where (schedulingid=?)";
    $result = array();
	$result = $dbh->prepare($query);
	$result->execute(array($schd));
    $i = 0;
...
function get_jamb_candidate_details($regNo) {global $dbh;
    $param = array();
    $query = "select * from tbljamb where (RegNo = ?)";
    $result = $dbh->prepare($query);
...
function get_jamb_candidate_schedule_details($regNo, $testid) {global $dbh; //date venue starttime batch
    $param = array();
