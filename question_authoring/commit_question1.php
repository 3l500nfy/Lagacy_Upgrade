    ...
    try {
        ...
        for ($i = 1; $i <= $qcount; $i++) {
            if (isset($_POST['q' . $i])) {
                ...
                $sql = "insert into tblquestionbank (title, difficultylevel, questiontime, active, author, subjectid , topicid) values ((select title from tblquestionbank_temp where questionbankid='$question'),'$difflvl',now(), 1, '".$_SESSION['MEMBER_USERID']."',$subjectid, '$topicid')";
