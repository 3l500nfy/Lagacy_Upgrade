...
$query = "SELECT * FROM tblscheduledcandidate WHERE RegNo='$username' order by candidatetype desc";
...
if($type==3){
    //regular student
    $qry = "SELECT loginpassword FROM tblstudents WHERE matricnumber='$username' AND loginpassword='$password' limit 1";
    
}elseif($type==2){
    //sbrs regular students
 $qry = "SELECT loginpassword FROM tblsbrsstudents WHERE sbrsno='$username' AND loginpassword='$password' limit 1";
    
}elseif($type==1){
 //jamb student
 $qry = "SELECT StateOfOrigin FROM tbljamb WHERE RegNo='$username' AND StateOfOrigin='$password' limit 1";
//echo $qry; exit;
    
}else{
    // sbrs  entrance candidate. modify query when table available
     $qry = "SELECT stateOforigin from tblsbrsshortlist where ApplicationID='$username' AND stateOforigin='$password' limit 1";
...
if ($numrows == 1) {
   ...
   //check if the student registered for the exams he is trying to write
		...
		$qry = "SELECT distinct candidateid, venueid FROM  tblcandidatestudent
		 INNER JOIN tblscheduling ON tblcandidatestudent.scheduleid= tblscheduling.schedulingid
		 WHERE matric ='$username' AND testid='$testid'";
		$stmt = $dbh->prepare($qry);
		$stmt->execute();
		$numrows = $stmt->rowCount();
